...
export async function POST(request: NextRequest) {
  ...
  if (!connectorKey) return jsonError('Missing connectorKey', 400);
  if (!sourceKind) return jsonError('Missing sourceKind', 400);
  ...
  const [created] = await db
    ...
    .values({
      ...
      config: typeof body.config === 'object' ? (body.config as any) : null,
