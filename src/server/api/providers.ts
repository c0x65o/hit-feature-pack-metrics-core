...
function loadAllIngestors(): IngestorConfig[] {
...
function listBackfillFilenames(cfg: IngestorConfig, limit = 500): string[] {
...
function requiredPartnerFields(partnerId: string) {
...
async function computeProviderRow(cfg: IngestorConfig) {
  ...
  let mappingLinkType: string | null = null;
  ...
  if (cfg.backfill?.enabled && cfg.backfill.kind === 'directory' && mapping?.kind === 'metrics_links') {
    ...
    if (validate && mappingLinkType && mappingKey === 'exact_filename' && fileNames.length > 0) {
      const existing = await db
        ...
        .from(metricsLinks)
        .where(and(eq(metricsLinks.linkType, mappingLinkType), inArray(metricsLinks.linkId as any, fileNames as any)) as any)
        .limit(1000);
  ...
  let lastPointDate: string | null = null;
  let lastUpdatedAt: string | null = null;
  let dataSourcesCount: number | null = null;
  ...
  let lastBatchFile: string | null = null;
  if (dataSourceId) {
    const b = await db
      .select({
