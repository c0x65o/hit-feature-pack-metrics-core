...
import { loadPartnerDefinitions, interpolateTemplate, type PartnerDefinition } from '../lib/partners';
...
export async function POST(request: NextRequest, ctx: { params: { id: string } }) {
  ...
  if (!def.verify) return jsonError('No verify action configured for this partner', 400);

  const db = getDb();
  const rows = await db.select().from(metricsPartnerCredentials).where(eq(metricsPartnerCredentials.id, id)).limit(1);
  const cred = rows[0];
  if (!cred) return jsonError('Partner is not configured (no credentials saved)', 400);
  if (!cred.enabled) return jsonError('Partner is disabled', 400);
