...
type TargetResolveResult = { assignment?: any | null };
...
async function callSameOrigin(request: NextRequest, path: string, init: RequestInit): Promise<Response> {
  ...
  if (rawToken) headers.set('X-HIT-Token-Raw', rawToken);
  const cookie = request.headers.get('cookie') || request.headers.get('Cookie');
  if (cookie) headers.set('Cookie', cookie);
  if (!headers.has('X-Frontend-Base-URL')) {
    const origin = request.headers.get('origin') || url.origin;
    if (origin) headers.set('X-Frontend-Base-URL', origin);
  }
...
async function resolveTargetValue(args: {
  ...
  const json = (await res.json().catch(() => ({}))) as TargetResolveResult;
...
async function latestMetricSum(args: {
  ...
  const whereParts: any[] = [eq(metricsMetricPoints.metricKey, args.metricKey)];
  if (args.start) whereParts.push(gte(metricsMetricPoints.date, new Date(args.start)));
  if (args.end) whereParts.push(lte(metricsMetricPoints.date, new Date(args.end)));
  if (args.entityKind) whereParts.push(eq(metricsMetricPoints.entityKind, args.entityKind));
  if (args.entityId) whereParts.push(eq(metricsMetricPoints.entityId, args.entityId));
  if (Array.isArray(args.entityIds) && args.entityIds.length > 0) {
    whereParts.push(inArray(metricsMetricPoints.entityId as any, args.entityIds as any));
  }
  if (args.dimensions) {
  ...
  const latestRows = await args.db
    .select({ maxDate: sql<Date | null>`max(${metricsMetricPoints.date})` })
  ...
  const sumRows = await args.db
    .select({ total: sql<number>`sum(${metricsMetricPoints.value})` })
    .from(metricsMetricPoints)
...
export async function tryRunComputedMetricQuery(args: {
  ...
  const entityId =
    String(args.body.dimensions?.user_id || args.body.entityId || (Array.isArray(args.body.entityIds) ? args.body.entityIds[0] : '') || '') ||
    '__all__';
