...
export async function tryRunComputedMetricQuery(args: {
  ...
  if (snapshotKeys.has(metricKey)) {
    ...
    if (agg === 'last') return { ok: false, error: 'agg=last is not supported for this CRM snapshot metric (use agg=sum/count)' };
  ...
  if (isPipelineStageCount || isPipelineStageValue) {
    ...
    if (agg === 'last') return { ok: false, error: 'agg=last is not supported for CRM pipeline-by-stage metrics' };
    const where: SQL[] = [];
    const stageFilterRes = applyStageDimFilters(dimFilters, where);
    if (!stageFilterRes.ok) return { ok: false, error: stageFilterRes.error };
  ...
  if (isLikelihoodCount || isLikelihoodValue) {
    ...
    if (agg === 'last') return { ok: false, error: 'agg=last is not supported for CRM likelihood metrics' };
  ...
  if (isStalledCount || isStalledValue) {
    ...
    if (agg === 'last') return { ok: false, error: 'agg=last is not supported for stalled opportunity metrics' };
