...
function loadIngestorFromDir(dir, id) {
    ...
    try {
        ...
        async function flushChunk() {
            ...
            if (chunk.length !== raw.length) {
                console.warn(`[metrics-upload] Deduped ${raw.length - chunk.length} duplicate metric_point row(s) within an insert batch for file="${fileName}".`);
            }
            await db
