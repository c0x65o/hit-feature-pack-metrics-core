...
export async function GET(request) {
    ...
    if (!includeInactive)
        where.push(eq(metricsSegments.isActive, true));
    if (q) {
        // naive search across key/label/description
        where.push(or(ilike(metricsSegments.key, `%${q}%`), ilike(metricsSegments.label, `%${q}%`), ilike(metricsSegments.description, `%${q}%`)));
    }
...
export async function POST(request) {
    ...
    if (existing.length)
        return jsonError('Segment key already exists', 409);
    const now = new Date();
    const id = makeId('seg');
    const row = {
        id,
    ...
    await db.insert(metricsSegments).values(row);
