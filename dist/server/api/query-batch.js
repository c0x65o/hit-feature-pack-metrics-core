...
export async function POST(request) {
    ...
    const pointKeyRows = metricKeys.length > 0
        ? await db
            .select({ metricKey: metricsMetricPoints.metricKey })
            .from(metricsMetricPoints)
            .where(inArray(metricsMetricPoints.metricKey, metricKeys))
            .groupBy(metricsMetricPoints.metricKey)
        : [];
    const keysWithPoints = new Set(pointKeyRows.map((r) => String(r?.metricKey || '').trim()).filter(Boolean));
