...
import { tryRunComputedMetricDrilldown } from '../lib/computed-metrics';
...
function addBucketEnd(start, bucket) {
...
function buildWhereFromPointFilter(filter) {
    const metricKey = String(filter.metricKey || '').trim();
    if (!metricKey)
        throw new Error('Missing metricKey');
    ...
    const entityIds = Array.isArray(filter.entityIds) ? filter.entityIds.map((x) => String(x || '').trim()).filter(Boolean) : [];
    ...
    if (entityIds.length) {
        if (entityIds.length > 1000)
            throw new Error('Too many entityIds (max 1000)');
    ...
    if (sourceGranularity)
        whereParts.push(eq(metricsMetricPoints.granularity, sourceGranularity));
    if (dimensions) {
        for (const [k, v] of Object.entries(dimensions)) {
            if (!/^[a-zA-Z0-9_]+$/.test(k))
                throw new Error(`Invalid dimensions key: ${k}`);
            if (v === null)
...
function derivePointFilterFromAggregate(input) {
    const base = input.baseQuery;
    const row = input.rowContext || {};
    const metricKey = String(base.metricKey || '').trim();
    if (!metricKey)
        throw new Error('Missing baseQuery.metricKey');
    const bucket = (base.bucket || 'day');
    ...
    if (bucket !== 'none' && rowBucket) {
        ...
        if (Number.isNaN(bs.getTime()))
            throw new Error('Invalid rowContext.bucket');
        start = bs;
    ...
    if (groupByEntityId && !entityId)
        throw new Error('Missing rowContext.entityId for groupByEntityId drilldown');
    const entityIds = Array.isArray(base.entityIds) ? base.entityIds.map((x) => String(x || '').trim()).filter(Boolean) : [];
    ...
    const dims = {};
    if (base.dimensions && typeof base.dimensions === 'object')
        Object.assign(dims, base.dimensions);
    ...
    if (groupBy.length) {
        if (!rowDims)
            throw new Error('Missing rowContext.dimensions for groupBy drilldown');
        for (const k of groupBy) {
            if (typeof k !== 'string' || !/^[a-zA-Z0-9_]+$/.test(k))
                throw new Error(`Invalid groupBy key: ${String(k)}`);
            if (!(k in rowDims))
                throw new Error(`rowContext.dimensions missing groupBy key: ${k}`);
            dims[k] = rowDims[k];
        }
    }
...
export async function POST(request) {
    ...
    try {
        ...
        if (!hasPoint.length) {
            ...
            const computed = await tryRunComputedMetricDrilldown({
                db,
                pointFilter: resolvedPointFilter,
                page: body.page,
    ...
    if (body.includeContributors) {
        ...
        const valueSum = sql `sum(${metricsMetricPoints.value})::float8`.as('valueSum');
        const pointCount = sql `count(*)::int`.as('pointCount');
        ...
        let byDimension = null;
        const dimKey = Array.isArray(deriveMeta?.groupBy) && deriveMeta.groupBy.length
            ? String(deriveMeta.groupBy[0])
            : null;
        if (dimKey && /^[a-zA-Z0-9_]+$/.test(dimKey)) {
            const dimExpr = sql `${metricsMetricPoints.dimensions} ->> ${dimKey}`.as(dimKey);
