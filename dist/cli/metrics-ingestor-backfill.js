...
function resolveProjectSchemaName() {
...
function loadServiceTokensFromFile() {
...
function resolveServiceToken() {
    ...
    if (serviceName && typeof tokens[serviceName] === 'string')
        return String(tokens[serviceName] || '').trim();
    if (typeof tokens.web === 'string')
        return String(tokens.web || '').trim();
...
function missingMappingsError(linkType, missing) {
    return new Error(`Missing required metrics_links mappings for ${missing.length} file(s).\n` +
...
async function validateMappingsViaApi(args, linkType, fileNames) {
    const missing = [];
    for (const name of fileNames) {
        ...
        const { res, bodyText } = await fetchWithRetry(url, { method: 'GET', headers: buildAuthHeaders(args) }, { retries: 20, baseDelayMs: 300 });
        if (!res.ok) {
        ...
        if (pg?.Client) {
            ...
            try {
                ...
                for (let i = 0; i < fileNames.length; i += chunkSize) {
                    ...
                    const res = await client.query(`select link_id from metrics_links where link_type = $1 and link_id = any($2::text[])`, [linkType, chunk]);
...
async function uploadOne(args, ingestorId, filePath) {
    ...
    const { res, bodyText: text } = await fetchWithRetry(url, { method: 'POST', headers: buildAuthHeaders(args), body: form }, { retries: 20, baseDelayMs: 400 });
    if (!res.ok) {
        ...
        const correlationId = typeof parsed?.correlationId === 'string' && parsed.correlationId ? parsed.correlationId : '';
        const errMsg = typeof parsed?.error === 'string' && parsed.error ? parsed.error : '';
