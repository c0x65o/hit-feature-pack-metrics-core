...
async function main() {
    ...
    for (const p of normalized) {
        ...
        if (!p.metricKey)
            throw new Error('Point missing metricKey');
        if (!p.date)
...
function parseArgs(argv) {
    ...
    for (let i = 0; i < opts.length; i++) {
        ...
        if (a === '--base-url')
            ...
            out.entityId = next();
        else if (a === '--connector-key')
            out.connectorKey = next();
        else if (a === '--source-kind')
            out.sourceKind = next();
        else if (a === '--external-ref')
            out.externalRef = next();
        else
